...
class ConvBnAct(nn.Module):
    ...
    def __init__(self, in_channel, num_class, kernel=3, stride=1, padding=1, dilation=1, bias=False, act=True):
...
class FastSPP(nn.Module):
    ...
    """
    def __init__(self, in_channel, num_class, kernel=5):
        super().__init__()
        mid_channel = in_channel // 2
        self.cba1 = ConvBnAct(in_channel, mid_channel, 1, 1, padding=0, dilation=1)
        self.cba2 = ConvBnAct(mid_channel * 4, num_class, 1, 1, padding=0, dilation=1)
...
class BasicBottleneck(nn.Module):
    ...
    def __init__(self, in_channel, num_class, shorcut, groups=1, expand_ratio=0.5):
        ...
        self.conv_bn_act_2 = ConvBnAct(mid_channel, num_class, 3, 1, 1, groups=groups)  # padding = 1, 不改变输入tensor的h,w
        self.residual = shorcut and (in_channel == num_class)
...
class C3BottleneckCSP(nn.Module):
    ...
    """
    def __init__(self, in_channel, num_class, shortcut=True, num_block=1, groups=1, bias=False):
        super(C3BottleneckCSP, self).__init__()
        mid_channel = num_class // 2
        ...
        self.cba3 = ConvBnAct(mid_channel*2, num_class, 1, 1, groups=groups, bias=bias)
...
class ResidualUBlock7(nn.Module):
    ...
    def __init__(self, in_channel, mid_channel, num_class) -> None:
        ...
        self.input_conv = ConvBnAct(in_channel, num_class)

        self.encoder1_conv = ConvBnAct(num_class, mid_channel)
        ...
        self.decoder1_conv = ConvBnAct(2 * mid_channel, num_class)
